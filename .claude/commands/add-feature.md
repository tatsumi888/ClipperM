...
1. 現在のタスクコンテキストを確立する:
  ...
  - ステアリングディレクトリパス: `.steering/[日付]-[連番]-[機能名]/`
...
1. `docs/` 内のリポジトリ構造定義書(`repository-structure.md` / `04-repository-structure.md` 等)でソースディレクトリを確認し(未定義の場合は`src/`)、Grepツールで機能名に関連するキーワードを検索する。
  ```bash
  Grep('[機能に関連するキーワード]', '[ソースディレクトリ]')
  ```
...
1. `docs/` 内の開発ガイドライン(`development-guidelines.md` / `05-development-guidelines.md` 等)に定義された検証コマンド(テスト・静的解析・型チェック)を順番に実行し、全てパスすることを確認する。未定義の場合は以下を実行する。
  ```bash
  Bash('npm test')
  Bash('npm run lint')
...
- ステップ7: 開発ガイドラインに定義された検証コマンド(テスト・静的解析・フォーマットチェック)が全てエラーなく成功する。
