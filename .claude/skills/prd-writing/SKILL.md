...
**重要**: `docs/` に既存のPRDがある場合(ファイル名は `product-requirements.md` でも
`01-product-requirements.md` でも、番号の有無・名前の細部を問いません)、
以下の優先順位に従ってください:
...
**更新時**: 既存PRDの構造と内容を維持しながら、既存のファイル名のまま更新する

## 出力先

**まず `docs/` を一覧してください。** 同種のドキュメント(PRD)が既にある場合は、番号の有無や
