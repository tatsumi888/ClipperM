...
**配置ドキュメント**(作成順の連番を付ける。既存プロジェクトで番号なしのファイル名が
既に使われている場合はこの表記に合わせてリネームせず、実際のファイル名に読み替える):
- `01-product-requirements.md`: プロダクト要求定義書
- `02-functional-design.md`: 機能設計書
- `03-architecture.md`: アーキテクチャ設計書
- `04-repository-structure.md`: リポジトリ構造定義書(本ドキュメント)
