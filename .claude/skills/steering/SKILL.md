...
1. **ステアリングディレクトリの確認**
   ```
   現在の日付を取得し、`.steering/[YYYYMMDD]-[NN]-[機能名]/` の形式でディレクトリを作成

   - `[NN]` は**その日の作成順の2桁連番**(01〜)。同一日に複数タスクがあってもフォルダ名で時系列にソートできるようにするため
   - 既存の `.steering/` を確認し、同日付の最大連番+1を割り当てる(その日の最初のタスクは `01`)
   ```

2. **永続ドキュメントの確認**

   **まず `docs/` を一覧してください。** 番号の有無を問わず、以下があれば読む:
   - PRD(`product-requirements.md` / `01-product-requirements.md` 等)
   - 機能設計書(`functional-design.md` / `02-functional-design.md` 等)
   - アーキテクチャ設計書(`architecture.md` / `03-architecture.md` 等)
   - リポジトリ構造(`repository-structure.md` / `04-repository-structure.md` 等)
   - 開発ガイドライン(`development-guidelines.md` / `05-development-guidelines.md` 等)
   - 用語集(`glossary.md` / `06-glossary.md` 等)
...
3. **テンプレートからファイル作成**
   ...
   - `.claude/skills/steering/templates/requirements.md` → `.steering/[日付]-[連番]-[機能名]/requirements.md`
   - `.claude/skills/steering/templates/design.md` → `.steering/[日付]-[連番]-[機能名]/design.md`
   - `.claude/skills/steering/templates/tasklist.md` → `.steering/[日付]-[連番]-[機能名]/tasklist.md`
...
```
Read('.steering/[日付]-[連番]-[機能名]/tasklist.md')
```
...
**3-4. 実装を実行**
```
開発ガイドライン（docs/ 内の development-guidelines.md、番号の有無を問わない）に従って実装
```
