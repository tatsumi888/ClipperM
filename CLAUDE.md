...
**等倍表示が要るときの受け皿。** EPUB が固定レイアウトを使えなくなったため、Kindle に
ピクセル等倍で出す唯一の手段になっている（2026-08-24 実機確認）。

`MediaBox` に画像のピクセル数をそのまま入れる。pt 扱いなので物理的には巨大なページになるが、
Kindle は PDF を画面に合わせて表示するため、**縦横比が端末と一致していれば等倍になる**。

### 埋め込み方は EPUB と同じ判断

| | フィルタ | 色空間 | 理由 |
|---|---|---|---|
| グレースケール | `/FlateDecode` | `/DeviceGray` | **可逆**。ディザのドットを壊さない。1チャンネルなので PNG(RGBA) の約 1/3 |
| フルカラー | `/DCTDecode` | `/DeviceRGB` | Canvas が出した JPEG を**そのまま格納**。再エンコードしない |

**グレースケールで非可逆にしないこと。** ディザが潰れると PDF を選ぶ意味が無くなる。

### xref のオフセットはバイト位置を実測する

**ずれた PDF は多くのビューアが黙って白紙を出す。** 文字数から推測せず、積んだバイト数を数える。
文字列は latin1 で扱うこと（UTF-8 で数えるとマルチバイト文字のぶんオフセットがずれる）。
日本語タイトルは UTF-16BE + BOM の16進文字列にする。

`tests/pdf.test.ts` が xref の各エントリが実際に `N 0 obj` を指しているかを検証している。
EPUB で mimetype をバイト列で確かめたのと同じ位置づけの検査。

## 形式のトレードオフ（Kindle 側の仕様。解消できない）

| | カバー（スリープ画面） | 表示 | サイズ |
|---|---|---|---|
| EPUB | **○** | 少し小さい（Kindle が余白を付けて縮小） | 920KB/ページ |
| PDF | × | **画面ぴったり（等倍）** | **318KB/ページ** |

どちらを捨てても用途が損なわれるため、**両方出せるようにして送信時に選ぶ**。
既定は EPUB（従来の挙動）。選択は `localStorage` に永続化する。
