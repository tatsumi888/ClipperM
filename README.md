...
| ファイル選択から取り込み | ○ | **実機確認済** |
| **共有メニューから取り込み** | ○ | ×（技術的に不可） |
| 共有シートへ EPUB を渡す | ○ | **実機確認済** |
| Kindle での表示 | — | **実機確認済** |

iPhone は 2026-08-15 に実機で一通り確認済み。**iOS Safari が `application/epub+zip` の File を共有シートに渡せるかが最後まで未知だったが、問題なく通った。** メールアプリ経由で `@kindle.com` に送り、Kindle 側で余白なく表示されるところまで確認できている。
